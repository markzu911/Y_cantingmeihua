...
async function saveResultImageToSaas({
  ...
  if (!userId || userId === 'null') throw new Error('Invalid User ID');
  if (!toolId || toolId === 'null') throw new Error('Invalid Tool ID');
  ...
  if (!consume.success) throw new Error(consume.error || '积分扣除失败');
