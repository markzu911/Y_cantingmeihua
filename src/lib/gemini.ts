...
export async function analyzeRestaurantImage(base64Image: string, mimeType: string): Promise<AnalysisResult> {
  ...
  const response = await callGemini("gemini-3-flash-preview", payload);
  let result;
  try {
...
- Make the final image look highly realistic, spotless, and premium.`;
  ...
  const response = await callGemini("gemini-3.1-flash-image-preview", payload);
  
  let generatedImage = null;
  // Look into the raw response parts for inlineData
  const parts = response.raw?.candidates?.[0]?.content?.parts || [];
